...
function fetchM3U() {
  return new Promise((resolve, reject) => {
    ...
    https.get(PLAYLIST_URL, { headers: { "User-Agent": "Mozilla/5.0" } }, (res) => {
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/api/channels") {
    try {
      ...
      res.end(JSON.stringify({ channels, cached_at: new Date(cache.timestamp).toISOString(), total: channels.length }));
    } catch (e) {
  ...
  if (pathname === "/api/check") {
    ...
    try {
      ...
      const checkReq = mod.request(
        { host: checkUrl.hostname, path: checkUrl.pathname + checkUrl.search, method: "HEAD", timeout: 5000 },
        (checkRes) => {
...
server.listen(PORT, () => {
  console.log(`\n✅ StreamFree running at http://localhost:${PORT}`);
  console.log(`📡 Fetching playlist on first request...`);
